...
class MyTestCase(unittest.TestCase):
    def test_example(self):
        ...
        initialise_sampling(space, observations=[], functions=g, N=500, N_obs=100, MH_samples=100, eps=0)

    def test_example2(self):
        space = RefinedSpace([(0, 1), (0, 1)], ["p", "q"], ["Real", "Real"], [[[0, 0.5], [0, 0.5]]], [],
                             true_point=[0.82, 0.92])
        f = ["p**2-2*p+1", "2*q*p**2-2*p**2-2*q*p+2*p", "(-2)*q*p**2+p**2+2*q*p"]
        initialise_sampling(space, observations=[], functions=f, N=500, N_obs=100, MH_samples=100, eps=0)

    def test_given_observation(self):
        space = RefinedSpace([(0, 1), (0, 1)], ["p", "q"], ["Real", "Real"], [[[0, 0.5], [0, 0.5]]], [],
                             true_point=[0.82, 0.92])
        f = ["p**2-2*p+1", "2*q*p**2-2*p**2-2*q*p+2*p", "(-2)*q*p**2+p**2+2*q*p"]
        initialise_sampling(space, observations=[0, 2, 1, 2, 1, 0, 2, 1, 0, 1], functions=f, N=500, N_obs=100, MH_samples=100, eps=0)

    def test_given_data(self):
        space = RefinedSpace([(0, 1), (0, 1)], ["p", "q"], ["Real", "Real"], [[[0, 0.5], [0, 0.5]]], [],
                             true_point=[0.82, 0.92])
        f = ["p**2-2*p+1", "2*q*p**2-2*p**2-2*q*p+2*p", "(-2)*q*p**2+p**2+2*q*p"]
        initialise_sampling(space, observations=[0.2, 0.5, 0.3], functions=f, N=500, N_obs=100, MH_samples=100, eps=0)
