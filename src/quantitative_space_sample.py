...
def quantitative_sample(space, constraints, sample_size, compress=False, silent=True, save=False, debug: bool = False, progress=False):
    """ Samples the space in **sample_size** samples in each dimension and saves how far is the point from satisfying / not satisfying the constraints
    ...
    if save:
        ...
        pickle.dump(sampling, open(os.path.join(refinement_results, ("Quantitative_space_sampling_" + save).split(".")[0] + ".p"), "wb"))

    space.quantitative_sampling_took(time() - start_time)
    return sampling