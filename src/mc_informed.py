...
def call_data_informed_prism(N, parameters, data, alpha, n_samples, multiparam, seq):
    ...
    if seq:
        ...
        for i in range(len(data[N])):
            ...
            print()
        print("end=$SECONDS")
        print('echo "It took: $((end-start)) seconds." >> {}/{}_{}_{}_{}_seq.txt 2>&1'.format(prism_results, model, N,
                                                                                              alpha, n_samples))
...
def call_storm(N, parameters, data, alpha, n_samples, multiparam):
    ...
    for i in range(len(data[N])):
        ...
        if data[N][i] - margin(alpha, n_samples, data[N][i]) > 0:
            suffix = "{}-low".format(i)
            sys.stdout.write('./storm-pars --prism /{}/{}_{}.pm --prop "P>{}'.format(storm_models, model, N,
                                                                                     data[N][i] - margin(alpha,
                                                                                                         n_samples,
                                                                                                         data[N][i])))
            if i > 0:
            ...
            sys.stdout.write(
                '" --refine --printfullresult >> /{}/{}_{}_{}_{}_seq_{}.txt 2>&1'.format(storm_output, model, N, alpha,
                                                                                         n_samples, suffix))
        ...
        print()
    print("end=$SECONDS")
    print('echo "It took: $((end-start)) seconds." >> /{}/{}_{}_{}_{}_seq_{}.txt 2>&1'.format(storm_output, model, N,
